...
window.saveRecipe = function (element, recipeId) {
    const /** {String} */ isSaved = window.localStorage.getItem(`recipedocious${recipeId}`);
    ACCESS_POINT = `${ROOT}/${recipeId}`;
    
    if (!isSaved) {
        fetchData(cardQueries, function (data) {
            window.localStorage.setItem(`recipedocious${recipeId}`, JSON.stringify(data));
        ...
        window.localStorage.removeItem(`recipedocious${recipeId}`);
        element.classList.toggle("saved");
        element.classList.toggle("removed");
